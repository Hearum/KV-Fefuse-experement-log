...
launch_task() { local gpu=$1 mode=$2 alpha=$3 s=$4 e=$5; local al=${alpha//./p}; local lp="queue_${mode}_alpha${al}_qjy003_gpu${gpu}_seg_${s}_${e}"; nohup "$EXP/launch_attention_ablation.sh" --worker "$mode" "$gpu" "$s" "$e" "$alpha" > "$EXP/logs/${lp}.outer.log" 2>&1 < /dev/null & echo $! > "$EXP/logs/${lp}.pid"; echo "[$(date '+%F %T')] launched $mode alpha=$alpha seg=$s-$e gpu=$gpu pid=$(cat "$EXP/logs/${lp}.pid")"; }
while true; do
  ...
  echo "[$(date '+%F %T')] qjy003 queue pending=$pending launched=$launched"
