...
OUT = ROOT / "MOTIVATION_EXPERIMENTS/structured_kv_adapter/results/stage2_predictability_ladder_73examples.json"
L, H, F, R = 64, 8, 128, 64
MAX_FEATURES = 4 * F + 4
VARIANTS = {
    "position": list(range(4 * F, 4 * F + 4)),
    "token_kv_position": list(range(0, 2 * F)) + list(range(4 * F, 4 * F + 4)),
    "token_prefix_kv_position": list(range(MAX_FEATURES)),
}
