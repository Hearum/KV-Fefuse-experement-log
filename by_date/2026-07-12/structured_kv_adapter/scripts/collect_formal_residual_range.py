...
p=argparse.ArgumentParser();p.add_argument("--shard",type=int,required=True);p.add_argument("--num-shards",type=int,default=8);p.add_argument("--start-example",type=int,default=50);p.add_argument("--end-example",type=int,default=100);p.add_argument("--output-dir",required=True);a=p.parse_args()
...
qs,system,_,_=prepare_reflect_data(str(ROOT/"data/result_reflect.json"),tok,"/mnt/qjhs-sh-lab-01/models/bge-m3","qwen3",topk=10,max_main_questions=a.end_example,preprocess=False,recall_method=RecallMethod.BGE,preprocess_scope=PreprocessScope.GLOBAL)
...
for ex,q in enumerate(qs):
 if ex<a.start_example or ex>=a.end_example or (ex-a.start_example)%a.num_shards!=a.shard or not q.get("should_test",True):continue
