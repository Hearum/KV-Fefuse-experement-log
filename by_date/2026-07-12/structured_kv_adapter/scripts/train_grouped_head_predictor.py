...
def main():
    ...
    model=Predictor(32,8).cuda(); optimizer=torch.optim.AdamW(model.parameters(),lr=2e-3,weight_decay=1e-4)
    ...
    result={"architecture":"shared token/prefix encoder + per-layer/head rank8 output heads; rank64 K/V coefficients","parameters":sum(p.numel() for p in model.parameters()),"train_examples":train_examples,"validation_examples":val_examples,"test_examples":test_examples,"train_samples":len(train_data),"validation_samples":len(val_data),"best_epoch":best[1],"history":history,"test_document_instances":documents,"rows":rows}
    torch.save({"state_dict":best[2],"coefficient_std":train_std.half(),"basis":basis_model,"config":{"prefix_dim":32,"rank":64,"group_rank":8}},RESULTS/"grouped_head_predictor.pt")
    (RESULTS/"grouped_head_predictor.json").write_text(json.dumps(result,indent=2)+"\n"); print(json.dumps(result,indent=2))
