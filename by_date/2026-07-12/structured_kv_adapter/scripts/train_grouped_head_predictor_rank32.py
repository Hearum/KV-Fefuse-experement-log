...
class Predictor(nn.Module):
    def __init__(self, prefix_dim=32, group_rank=8):
        super().__init__()
        ...
        self.group_b = nn.Parameter(torch.empty(L, H, group_rank, 2 * R))
        self.group_bias = nn.Parameter(torch.zeros(L, H, 2 * R))
        nn.init.normal_(self.group_a, std=0.02)
        nn.init.zeros_(self.group_b)

    def forward(self, own, prefix, position, layer, head):
        features = torch.cat([self.own(own), self.prefix(prefix), position], dim=-1)
...
def sampled_dataset(paths, wanted_split, basis_model, samples_per_doc=64):
    ...
    for path in paths:
        ...
        for rank, item in enumerate(record["items"]):
            ...
            if samples_per_doc % (L * H) != 0:
                raise ValueError("samples_per_doc must cover every layer/head pair")
            repeats = samples_per_doc // (L * H)
            layers = torch.arange(L).view(L, 1).expand(L, H).reshape(-1).repeat(repeats)
            heads = torch.arange(H).view(1, H).expand(L, H).reshape(-1).repeat(repeats)
...
def main():
    ...
    train_data,train_std,train_examples=sampled_dataset(paths,"train",basis_model,512)
    val_data,_,val_examples=sampled_dataset(paths,"validation",basis_model,512)
    coefficient_std=train_std.cuda()
    train_loader=DataLoader(train_data,batch_size=8192,shuffle=True,num_workers=0,pin_memory=True)
    val_loader=DataLoader(val_data,batch_size=8192,shuffle=False,num_workers=0,pin_memory=True)
    model=Predictor(32,32).cuda(); optimizer=torch.optim.AdamW(model.parameters(),lr=2e-3,weight_decay=1e-4)
    ...
    result={"architecture":"shared token/prefix encoder + per-layer/head rank32 output heads; rank64 K/V coefficients","parameters":sum(p.numel() for p in model.parameters()),"train_examples":train_examples,"validation_examples":val_examples,"test_examples":test_examples,"train_samples":len(train_data),"validation_samples":len(val_data),"best_epoch":best[1],"history":history,"test_document_instances":documents,"rows":rows}
    torch.save({"state_dict":best[2],"coefficient_std":train_std.half(),"basis":basis_model,"config":{"prefix_dim":32,"rank":64,"group_rank":32}},RESULTS/"grouped_head_predictor_rank32.pt")
    (RESULTS/"grouped_head_predictor_rank32.json").write_text(json.dumps(result,indent=2)+"\n"); print(json.dumps(result,indent=2))
