...
ROOT = Path.cwd()
EXP = Path(os.environ.get("FUSIONRAG_UNIFORM_A01_RATE_SWEEP_ROOT", "/mnt/qjhs-sh-lab-04/lpl/hming/qwen3_attention_uniform_a01_rate_sweep"))
LOGS = EXP / "logs"
...
CACHE_ROOT = Path(os.environ.get("FUSIONRAG_UNIFORM_A01_CACHE_ROOT", "/mnt/qjhs-sh-lab-04/lpl/hming/fusionrag-qwen3-uniform-a01-rate-sweep-cache"))
WORKER_GPUS = [int(x) for x in os.environ.get("FUSIONRAG_UNIFORM_A01_WORKER_GPUS", "1,2,3,4,5,6,7").split(",") if x.strip()]
RATES = [x.strip() for x in os.environ.get("FUSIONRAG_UNIFORM_A01_RATES", "0.3,0.5,0.8,1.0").split(",") if x.strip()]
CSV_BY_RATE = {r: f"rate_{r}_draft_Qwen2.5-3B-Instruct_revert_rope.csv" for r in RATES}
...
def build_tasks():
    include = os.environ.get("FUSIONRAG_UNIFORM_A01_DATASETS", "2wikimqa,hotpotqa,triviaqa,musique")
    ...
    for rate in RATES:
        for ds in wanted:
            ...
            for start, end in segments(n):
                tasks.append({"rate": rate, "dataset": ds, "data_path": data_path, "start": start, "end": end})
    return tasks

def label(rate):
    return f"uniform_alpha0p1_rate{rate.replace('.', 'p')}"

def out_dir(task):
    return EXP / label(task["rate"]) / task["dataset"] / f"seg_{task['start']}_{task['end']}"
...
def done(task):
    ...
    try:
        ...
        return False
    return any(d.glob(f"**/{CSV_BY_RATE[task['rate']]}"))
...
def worker(gpu, tasks):
    ...
    log_path = LOGS / f"uniform_a01_worker_{host}_gpu{gpu}.log"
    with log_path.open("a", encoding="utf-8", buffering=1) as log:
        log.write(f"[{time.strftime('%F %T')}] start gpu={gpu} tasks={len(tasks)} root={ROOT}\n")
        for task in tasks:
            name = f"rate={task['rate']} {task['dataset']} {task['start']}-{task['end']}"
            if done(task):
