...
OUT_ROOT="$REPO/MOTIVATION_EXPERIMENTS/qwen3_draft_random_score_rate015"
...
run_segment() {
  ...
  local start="$2"
  local end="$3"
  local out_dir="$OUT_ROOT/seg_${start}_${end}"
  mkdir -p "$out_dir" "$OUT_ROOT/logs"
  echo "[$(date '+%F %T')] start random draft score gpu=$gpu segment=$start-$end" | tee -a "$OUT_ROOT/logs/seg_${start}_${end}.log"
  cd "$REPO"
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 \
  FUSIONRAG_DRAFT_RANDOM_SCORE=1 FUSIONRAG_DRAFT_RANDOM_SEED=20260712 \
  "$PY" test_fusionrag_reflect_preprocess_exp.py \
    ...
    > "$out_dir/run.log" 2>&1
  echo "[$(date '+%F %T')] done random draft score gpu=$gpu segment=$start-$end" | tee -a "$OUT_ROOT/logs/seg_${start}_${end}.log"
}
