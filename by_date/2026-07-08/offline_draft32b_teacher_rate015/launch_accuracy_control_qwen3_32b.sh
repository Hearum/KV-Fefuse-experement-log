...
OUT_ROOT="$EXP/accuracy_runs_control_qwen3_32b"
FIXED="$EXP/fixed_sets_control_qwen3_32b/chunk_fixed_sets_npz"
...
run_cfg() {
  ...
  if [[ -f "$out_dir/run.log" ]] && grep -q "FINAL RESULTS" "$out_dir/run.log"; then
    echo "[$(date '+%F %T')] skip complete method=$method segment=$start-$end" | tee -a "$EXP/logs/accuracy_segments.log"
    return 0
  fi
  echo "[$(date '+%F %T')] start accuracy method=$method gpu=$gpu segment=$start-$end" | tee -a "$EXP/logs/accuracy_segments.log"
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 "$PY" test_fusionrag_reflect_preprocess_exp.py \
    ...
    > "$out_dir/run.log" 2>&1
  echo "[$(date '+%F %T')] done accuracy method=$method gpu=$gpu segment=$start-$end" | tee -a "$EXP/logs/accuracy_segments.log"
}
...
for gpu in 0 1 2 3 4 5 6 7; do
  ...
  nohup "$0" --worker "$gpu" "$start" "$end" > "$EXP/logs/accuracy_gpu${gpu}_${start}_${end}.outer.log" 2>&1 < /dev/null &
  echo $! > "$EXP/logs/accuracy_gpu${gpu}_${start}_${end}.pid"
  echo "launched accuracy gpu=$gpu segment=$start-$end pid=$(cat "$EXP/logs/accuracy_gpu${gpu}_${start}_${end}.pid")"
done
