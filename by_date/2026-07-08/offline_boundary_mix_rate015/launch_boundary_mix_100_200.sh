...
run_cfg() {
  ...
  if [[ -s "$csv" ]]; then
    echo "[$(date '+%F %T')] skip existing method=$method segment=$start-$end" | tee -a "$OUT_ROOT/logs/extra_gpu${gpu}.log"
    return
  fi
  echo "[$(date '+%F %T')] start method=$method gpu=$gpu segment=$start-$end" | tee -a "$OUT_ROOT/logs/extra_gpu${gpu}.log"
  cd "$REPO"
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 "$PY" test_fusionrag_reflect_preprocess_exp.py \
    ...
    > "$out_dir/run.log" 2>&1
  echo "[$(date '+%F %T')] done method=$method gpu=$gpu segment=$start-$end" | tee -a "$OUT_ROOT/logs/extra_gpu${gpu}.log"
}
...
for spec in "${workers[@]}"; do
  ...
  nohup "$0" --worker "$gpu" "$start" "$end" > "$OUT_ROOT/logs/extra_gpu${gpu}.outer.log" 2>&1 < /dev/null &
  echo $! > "$OUT_ROOT/logs/extra_gpu${gpu}.pid"
  echo "launched extra gpu=$gpu segment=$start-$end pid=$(cat "$OUT_ROOT/logs/extra_gpu${gpu}.pid")"
done
