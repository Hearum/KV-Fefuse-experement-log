...
run_one() {
  ...
  echo "[$(date '+%F %T')] START host=$(hostname) dataset=$dataset method=$method_label rate=$rate" | tee -a "$EXP/logs/qjy000_resume_queue.log"
  cd "$REPO"
  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 PYTHONUNBUFFERED=1 "$PY" test_fusionrag_reflect_preprocess_exp.py \
    ...
    "${extra[@]}" \
    > "$out_dir/run.log" 2>&1
  echo "[$(date '+%F %T')] DONE host=$(hostname) dataset=$dataset method=$method_label" | tee -a "$EXP/logs/qjy000_resume_queue.log"
}
...
echo "[$(date '+%F %T')] RESUME qjy000 queue: skip completed 2wikimqa full_rate1 and online_qk_rate015" | tee -a "$EXP/logs/qjy000_resume_queue.log"
