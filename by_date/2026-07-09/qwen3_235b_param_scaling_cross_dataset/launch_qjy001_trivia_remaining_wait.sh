...
LOG=$EXP/logs/qjy001_trivia_remaining_wait.log
...
wait_for_current_queue() {
  while tmux has-session -t qwen3_235b_param_qjy001_musique_2wiki 2>/dev/null; do
    echo "[$(date '+%F %T')] WAIT qjy001 current queue still running" | tee -a "$LOG"
    sleep 300
  done
}
...
echo "[$(date '+%F %T')] START qjy001 trivia remaining waiter" | tee -a "$LOG"
wait_for_current_queue
run_one online_draft_rate015
run_one offline3b_mean_rate015
cd "$EXP"
"$PY" summarize_results.py || true
echo "[$(date '+%F %T')] FINISHED qjy001 trivia remaining queue" | tee -a "$LOG"
