...
LOG=$EXP/logs/qjy003_triviaqa_only.log
...
echo "[$(date '+%F %T')] START qjy003 triviaqa-only queue" | tee -a "$LOG"
for method in \
  full_rate1 \
  online_qk_rate015 \
  online_draft_rate015 \
...
echo "[$(date '+%F %T')] FINISHED qjy003 triviaqa-only queue" | tee -a "$LOG"
