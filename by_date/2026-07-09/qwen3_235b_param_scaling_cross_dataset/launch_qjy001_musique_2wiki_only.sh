...
LOG=$EXP/logs/qjy001_musique_2wiki_only.log
...
echo "[$(date '+%F %T')] START qjy001 musique/2wiki offline32B queue" | tee -a "$LOG"
run_one musique "$REPO/data/result_reflect.json" 200 offline32b_top2_rate015
run_one 2wikimqa "$REPO/MOTIVATION_EXPERIMENTS/cross_dataset_offline_generalization/data/2wikimqa_reflect.json" 200 offline32b_top2_rate015

cd "$EXP"
"$PY" summarize_results.py || true
echo "[$(date '+%F %T')] FINISHED qjy001 musique/2wiki offline32B queue" | tee -a "$LOG"
