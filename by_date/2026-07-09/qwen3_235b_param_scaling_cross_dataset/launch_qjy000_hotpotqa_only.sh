...
LOG=$EXP/logs/qjy000_hotpotqa_only.log
...
run_one() {
  ...
  local data_path="$REPO/MOTIVATION_EXPERIMENTS/cross_dataset_offline_generalization/data/hotpotqa_reflect.json"
  local end_sample=260
...
echo "[$(date '+%F %T')] START qjy000 hotpotqa-only queue" | tee -a "$LOG"
for method in \
  full_rate1 \
  online_qk_rate015 \
  online_draft_rate015 \
...
echo "[$(date '+%F %T')] FINISHED qjy000 hotpotqa-only queue" | tee -a "$LOG"
