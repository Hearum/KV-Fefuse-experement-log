...
LOG=$EXP/logs/qjy001_all_remaining_wait.log
...
wait_for_qjy001_235b_queue() {
  while tmux has-session -t qwen3_235b_param_qjy001_trivia_wait 2>/dev/null; do
    echo "[$(date '+%F %T')] WAIT qjy001 current 235B queue still running" | tee -a "$LOG"
    sleep 300
  done
}
...
echo "[$(date '+%F %T')] START qjy001 all-remaining 235B waiter" | tee -a "$LOG"
wait_for_qjy001_235b_queue
...
echo "[$(date '+%F %T')] FINISHED qjy001 all-remaining 235B queue" | tee -a "$LOG"
