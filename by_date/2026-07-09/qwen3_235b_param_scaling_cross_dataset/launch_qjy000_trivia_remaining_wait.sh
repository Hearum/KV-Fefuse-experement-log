...
LOG=$EXP/logs/qjy000_trivia_remaining_wait.log
...
wait_for_current_queue() {
  while tmux has-session -t qwen3_235b_param_qjy000_hotpotqa 2>/dev/null; do
    echo "[$(date '+%F %T')] WAIT qjy000 current queue still running" | tee -a "$LOG"
    sleep 300
  done
}
...
run_one() {
  ...
  local data_path="$REPO/MOTIVATION_EXPERIMENTS/cross_dataset_offline_generalization/data/triviaqa_reflect.json"
  local end_sample=270
...
echo "[$(date '+%F %T')] START qjy000 trivia remaining waiter" | tee -a "$LOG"
wait_for_current_queue
run_one offline3b_freq_boundary2_rate015
run_one offline32b_top2_rate015
cd "$EXP"
"$PY" summarize_results.py || true
echo "[$(date '+%F %T')] FINISHED qjy000 trivia remaining queue" | tee -a "$LOG"
