...
FIXED="$EXP/docgen_chunk_draft_smart_full/rate_0p1/chunk_fixed_sets_npz"
...
run_segment() {
  ...
  local out_dir="$OUT_ROOT/offline10_docgen_draft005/seg_${start}_${end}"
  mkdir -p "$out_dir" "$OUT_ROOT/logs"
  cd "$REPO"
  echo "START offline10_docgen_draft005 gpu=$gpu seg=$start-$end $(date)"
  ...
  "$PY" test_fusionrag_reflect_preprocess_exp.py \
    ...
    --offline_fixed_set_method docgen_draft_smart_frequency_global \
    --offline_fixed_set_rate 0.10 \
    > "$out_dir/run.log" 2>&1
  echo "DONE offline10_docgen_draft005 gpu=$gpu seg=$start-$end $(date)"
}
...
for gpu in 0 1 2 3 4 5 6 7; do
  ...
  nohup "$0" --worker "$gpu" "$start" "$end" > "$OUT_ROOT/logs/offline10_docgen_draft005_seg_${start}_${end}.outer.log" 2>&1 < /dev/null &
  echo $! > "$OUT_ROOT/logs/offline10_docgen_draft005_seg_${start}_${end}.pid"
  echo "launched gpu=$gpu seg=$start-$end pid=$(cat "$OUT_ROOT/logs/offline10_docgen_draft005_seg_${start}_${end}.pid")"
done
