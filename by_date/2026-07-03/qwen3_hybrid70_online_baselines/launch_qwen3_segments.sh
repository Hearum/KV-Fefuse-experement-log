...
OUT_ROOT="$REPO/MOTIVATION_EXPERIMENTS/qwen3_hybrid70_online_baselines"
FIXED="$REPO/MOTIVATION_EXPERIMENTS/offline_fixed_token_selector/reflect_rate_sweep_from_qk_draft_scores/rate_0p5/chunk_fixed_sets_npz"
...
run_cfg() {
  ...
  if [[ "$cfg" == "full_rate1" ]]; then
    rate="1.0"
  elif [[ "$cfg" == "online_qk_rate050" ]]; then
    rate="0.5"
  elif [[ "$cfg" == "online_draft_rate050" ]]; then
    rate="0.5"
    method="DraftModel"
    extra+=(--draft_model_path "$DRAFT")
  elif [[ "$cfg" == "offline_hybrid70_rate050" ]]; then
    rate="0.5"
    extra+=(--offline_fixed_set_dir "$FIXED" --offline_fixed_set_method hybrid_draft70_qk30_score_per_chunk --offline_fixed_set_rate 0.5)
  else
...
run_segment() {
  ...
  local start="$2"
  local end="$3"
  run_cfg "$gpu" "$start" "$end" full_rate1
  run_cfg "$gpu" "$start" "$end" online_qk_rate050
  run_cfg "$gpu" "$start" "$end" online_draft_rate050
  run_cfg "$gpu" "$start" "$end" offline_hybrid70_rate050
}
