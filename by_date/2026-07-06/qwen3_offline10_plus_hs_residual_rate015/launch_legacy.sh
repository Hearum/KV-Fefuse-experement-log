...
OUT_ROOT="$REPO/MOTIVATION_EXPERIMENTS/qwen3_offline10_plus_hs_residual_rate015"
...
run_segment() {
  ...
  local out_dir="$OUT_ROOT/offline10_hs005_epoch003/seg_${start}_${end}"
...
- Method name: FusionRAG-HS (Hidden-Scorer Distillation)
- Base: Qwen3-32B main model
- Dataset: MuSiQue reflect 200 examples
- Offline fixed set: draft_smart_frequency_global, 10%
- Online residual: HiddenScorerSelector, 5%, checkpoint epoch003
- Total budget: 15%
- GPUs: 4,5,6,7 only, because GPUs 0-3 are occupied by sglang services.
- Launch: \`bash MOTIVATION_EXPERIMENTS/qwen3_offline10_plus_hs_residual_rate015_launch.sh\`
- Smoke passed: \`MOTIVATION_EXPERIMENTS/qwen3_offline10_plus_hs_residual_rate015_smoke_gpu4_retry2\`.

## Status

Launched at $(date +%F' '%T). Results will be under \`offline10_hs005_epoch003/seg_*\`.
MD
