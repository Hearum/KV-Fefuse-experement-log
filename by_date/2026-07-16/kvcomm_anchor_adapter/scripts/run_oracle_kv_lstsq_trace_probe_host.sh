...
TAG=oracle_kv_lstsq_retrieval_trace_a12_pdocs3_ridge1e-4_seed20260716
HOST=$(hostname)
LOG_ROOT="$EXP/results/oracle_span_trace_probe/logs"
mkdir -p "$LOG_ROOT"
...
run_shard() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 "$PY" \
    ...
    --source preprocess --example "$ex" --prefix-source retrieval_traces \
    --trace-anchors 12 --prefix-docs 3 --topk 10 --retrieval-topk 50 \
