# Setup-Standard V2 Cross-Dataset Evaluation

本目录用于重新按 `FusionRAG-setup.md` 的原始数据格式评测 Qwen3-32B，而不是使用 reflect/main-sub wrapper 数据。数据集统一标记为 `*-v2`，用于和之前 reflect 版本区分。

## 目标

重新构建四个 setup-standard 数据集：

| v2 dataset | setup 原始文件 | 说明 |
|---|---|---|
| `musique-v2` | `/mnt/qjhs-sh-lab-01/wjh/FusionRAG/data/musique-200.jsonl` | 必须重跑；之前 reflect 版有 main/sub-question 分解，任务格式不同。 |
| `2wikimqa-v2` | `/mnt/qjhs-sh-lab-01/wjh/FusionRAG/data/2wikimqa-200.jsonl` | 建议重跑；之前 reflect 文档集合和 setup 原始 context 不完全等价。 |
| `hotpotqa-v2` | `/mnt/qjhs-sh-lab-01/wjh/FusionRAG/data/hotpotqa-260-100-10-doc.jsonl` | setup 原始 `output[0].document` 格式。 |
| `triviaqa-v2` | `/mnt/qjhs-sh-lab-01/wjh/FusionRAG/data/triviaqa-270-100-10-doc.jsonl` | setup 原始 `output[0].document` 格式。 |

需要收集的方法：

| group | method label | rate sweep | 当前状态 |
|---|---|---|---|
| full | `full_rate1` | `1.0` | 可直接用 setup-format runner 跑。 |
| online QK | `online_qk` | `0,0.15,0.3,0.5,0.8,1.0` | 可直接跑。 |
| online DraftModel | `online_draft` | `0,0.15,0.3,0.5,0.8,1.0` | 可直接跑，draft=`Qwen2.5-3B-Instruct`。 |
| attention ablation | `uniform_alpha0p1_draft` | `0.15,0.3,0.5,0.8,1.0` | 可直接跑，环境变量控制重算 attention。 |
| offline fixed set | `offline3b_mean` | `0.15` first | 需要先把 fixed-set selection 接到 setup-format pipeline。 |
| offline fixed set | `offline3b_freq_boundary2` | `0.15` first | 同上。 |
| offline fixed set | `offline32b_top2` | `0.15` first | 同上。 |

历史 reflect 版 GLM-clean 重判里，offline 三组的表现如下，作为 v2 候选配置来源，不作为 v2 最终结果：

| dataset | best historical offline candidates |
|---|---|
| `2wikimqa` | `offline32b_top2` 106/200=53.00%，`offline3b_mean`/`offline3b_freq_boundary2` 105/200=52.50%。 |
| `hotpotqa` | `offline3b_mean` 229/260=88.08%，`offline32b_top2` 227/260=87.31%，`offline3b_freq_boundary2` 224/260=86.15%。 |
| `triviaqa` | `offline3b_freq_boundary2` 244/270=90.37%，`offline3b_mean` 242/270=89.63%，`offline32b_top2` 240/270=88.89%。 |
| `musique` | reflect 版可比性差；历史 best 包括 `offline10_hybrid_old70_docgen30_draft005` 109/135=80.74%、`offline10_draft005` 108/135=80.00%。这些不是 setup-format fixed set，暂不直接复用。 |


## 与历史 pipeline 的口径差异

本实验使用 setup-standard v2 数据和 runner，和之前多个 cross-dataset / reflect 实验不是同一个 pipeline。后续引用结果时必须显式标注来源，不能把两者直接混在同一 baseline 表里。

主要差异：

- 历史 cross-dataset / reflect 实验多使用 reflect/main-sub wrapper 数据，MuSiQue 尤其包含 main/sub-question 分解。
- setup-standard v2 直接读取 `FusionRAG-setup.md` 对应的原始 jsonl/context 格式，用 `*-v2` 数据集名标记。
- 两者的数据路径、问题组织方式、document/context 字段、runner 入口和部分 cache 定义都可能不同。
- 若要比较 `online_qk`、`online_draft`、`full_rate1`、offline fixed set 或 attention ablation，必须在同一 pipeline 下重跑，不能用历史 reflect 表直接替代 setup-v2 表。

记录实验结果时至少写清楚：pipeline 名称、数据路径、runner 脚本、cache 路径、commit hash、启动命令。

## Cache 规范

所有 setup-v2 实验必须共用同一套 Qwen3-32B KV cache，不允许按 worker/GPU/实验名再复制 preprocess cache。

```text
/mnt/qjhs-sh-lab-03/lpl/hming/fusionrag-kv-cache/Qwen3-32B/setup-v2/
  data/musique-v2/Qwen3-32B/
  data/musique-v2-preprocess-10-revert_rope-True/Qwen3-32B/
  data/2wikimqa-v2/Qwen3-32B/
  data/2wikimqa-v2-preprocess-10-revert_rope-True/Qwen3-32B/
  ...
```

并发准则：

1. 先跑 cache warmup，生成 raw/preprocess KV。
2. warmup 完成后，24 张卡并行评测只读 cache。
3. 每个 worker 的结果 CSV 写入 `results/<method>/<dataset>/<rate>/seg_<start>_<end>/`，不写入 cache 目录。
4. 如果发现 shared cache 下有 0 字节 `.pt` 或 worker 分叉目录，先停止该 worker 并记录，不继续扩大。

## 当前实现口径

`scripts/run_setup_v2_task.py` 使用本仓库的 `ktransformers.unified_process_cache`，但读取 setup 原始 jsonl。它通过 monkey-patch `prepare_data` 把 `csv_path` 改到 segment 结果目录，同时保留 shared raw/preprocess cache 路径。

注意：原 setup-format `unified_process_cache.py` 尚不支持 reflect 入口里的 `--offline_fixed_set_dir`。因此 offline fixed-set v2 需要下一步补接口，不能直接复用 reflect fixed-set 文件。
...
/mnt/qjhs-sh-lab-01/wjh/FusionRAG/.venv/bin/python \
  MOTIVATION_EXPERIMENTS/setup_standard_v2_cross_dataset/scripts/build_v2_data.py
```
...
当前还未启动正式 v2 评测。先完成 smoke 与 cache warmup 后，再把 `scripts/summarize_setup_v2.py` 的输出追加到本节。

| dataset | method | rate | rows | EM | F1 | notes | source |
|---|---|---:|---:|---:|---:|---|---|
| `musique-v2` | pending | - | - | - | - | - | setup-v2 |
| `2wikimqa-v2` | pending | - | - | - | - | - | setup-v2 |
| `hotpotqa-v2` | pending | - | - | - | - | - | setup-v2 |
| `triviaqa-v2` | pending | - | - | - | - | - | setup-v2 |



## 2026-07-14 Smoke 结果

已完成 `musique-v2 / online_qk / rate=0.15 / example 0:1` 单样本 smoke：

```bash
cd /raid/home/hming/FusionRAG-pca-analysis
CUDA_VISIBLE_DEVICES=2 PYTHONUNBUFFERED=1 /mnt/qjhs-sh-lab-01/wjh/FusionRAG/.venv/bin/python   MOTIVATION_EXPERIMENTS/setup_standard_v2_cross_dataset/scripts/run_setup_v2_task.py   --dataset musique-v2   --method online_qk   --rate 0.15   --start 0   --end 1   --gpu 2   > MOTIVATION_EXPERIMENTS/setup_standard_v2_cross_dataset/logs/smoke_musique_online_qk_rate015_0_1.log 2>&1
```

输出：

- `smoke_results/online_qk/musique-v2/rate_0p15/seg_0_1/csv/reprocess_method_FusionRAG_rate_0.15_revert_rope_True_topk_10.csv`
- `setup_v2_summary.csv`
- shared cache: `/mnt/qjhs-sh-lab-03/lpl/hming/fusionrag-kv-cache/Qwen3-32B/setup-v2`

Smoke 表：

| dataset | method | rate | rows | EM | F1 | pred | gold |
|---|---|---:|---:|---:|---:|---|---|
| `musique-v2` | `online_qk` | 0.15 | 1 | 0.00 | 0.00 | `Salma Hayek` | `Maria Bello` |

Cache 检查：

- 当前 shared cache 大小：约 31G。
- 0 字节 `.pt`：0。

注意：原 setup `unified_process_cache.py` 的 txt 汇总在使用 `ONLY_EXAMPLES` 时仍以完整 `tokens_data` 长度作为 denominator，因此 smoke txt 里的 `num_in_batch: 200` 和 `EM Score: 0.0000` 不能作为 segment 汇总依据。正式表使用 `scripts/summarize_setup_v2.py` 从 CSV 行重算 EM/F1。

## 下一步执行顺序

1. 先实现 setup-format cache warmup：按 dataset 单独生成 raw/preprocess KV，避免 24 个 worker 同时补同一 cache。
2. warmup 完成后启动 `scripts/launch_setup_v2_workers.py` 的 24-slot online/full/alpha 队列。
3. 之后补 setup-format offline fixed-set 接口，再跑历史候选 `offline3b_mean`、`offline3b_freq_boundary2`、`offline32b_top2`。
