...
def build_tasks():
    ...
    shard_count = int(os.environ.get("FUSIONRAG_QWEN3_CROSS_RATE_SWEEP_SHARD_COUNT", "1") or "1")
    shard_index = int(os.environ.get("FUSIONRAG_QWEN3_CROSS_RATE_SWEEP_SHARD_INDEX", "0") or "0")
    if shard_count > 1:
...
def done(task: dict) -> bool:
    ...
    try:
        ...
        return False
    expected = csv_name(task["method"], task["rate"])
    return any(directory.glob(f"**/{expected}"))
