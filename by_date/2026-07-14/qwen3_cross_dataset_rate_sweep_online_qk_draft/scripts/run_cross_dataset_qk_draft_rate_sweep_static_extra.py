#!/usr/bin/env python3
"""Launch Qwen3-32B Online QK/DraftModel rate sweep across RAG datasets."""
...
DATASET_TOTALS = {"2wikimqa": 200, "hotpotqa": 260, "triviaqa": 270}
SEGMENT = 25
...
CACHE_ROOT = Path(os.environ.get(
    "FUSIONRAG_QWEN3_CROSS_RATE_SWEEP_CACHE_ROOT",
    "/mnt/qjhs-sh-lab-03/lpl/hming/fusionrag-kv-cache",
))
CACHE_LAYOUT = os.environ.get("FUSIONRAG_QWEN3_CROSS_RATE_SWEEP_CACHE_LAYOUT", "shared").strip().lower()
CACHE_READONLY = os.environ.get("FUSIONRAG_QWEN3_CROSS_RATE_SWEEP_CACHE_READONLY", "0").strip().lower()
LOG_DIR = EXP_ROOT / "logs"
LOG_DIR.mkdir(parents=True, exist_ok=True)

DATASETS = [x.strip() for x in os.environ.get("FUSIONRAG_QWEN3_CROSS_RATE_SWEEP_DATASETS", "2wikimqa,hotpotqa,triviaqa").split(",") if x.strip()]
RATES = [x.strip() for x in os.environ.get("FUSIONRAG_QWEN3_CROSS_RATE_SWEEP_RATES", "0.0,0.1,0.3,0.5,0.8,0.9").split(",") if x.strip()]
METHODS = [x.strip() for x in os.environ.get("FUSIONRAG_QWEN3_CROSS_RATE_SWEEP_METHODS", "online_qk,online_draft").split(",") if x.strip()]
GPUS = [int(x) for x in os.environ.get("FUSIONRAG_QWEN3_CROSS_RATE_SWEEP_GPUS", "0,1,2,3,4,5,6,7").split(",") if x.strip()]


def segments(dataset: str):
    start = 0
...
def method_cfg(method: str) -> dict:
    ...
    if method == "online_draft":
        return {"reprocess_method": "DraftModel", "subdir": "DraftModel_global_topk10_bge", "extra": ["--draft_model_path", DRAFT]}
    ...
    for dataset in DATASETS:
        ...
        for rate in RATES:
            for method in METHODS:
                method_cfg(method)
                for start, end in segments(dataset):
                    tasks.append({"dataset": dataset, "rate": rate, "method": method, "start": start, "end": end})
    stride = int(os.environ.get("FUSIONRAG_QWEN3_CROSS_RATE_SWEEP_STRIDE", "1") or "1")
    offset = int(os.environ.get("FUSIONRAG_QWEN3_CROSS_RATE_SWEEP_OFFSET", "0") or "0")
    if stride > 1:
        tasks = [task for idx, task in enumerate(tasks) if idx % stride == offset]
    return tasks


def out_dir(task: dict) -> Path:
    rate_label = task["rate"].replace(".", "p")
    return EXP_ROOT / f"{task['method']}_rate{rate_label}" / task["dataset"] / f"seg_{task['start']}_{task['end']}"


def done(task: dict) -> bool:
    directory = out_dir(task)
    log = directory / "run.log"
    # Static-extra workers must not duplicate tasks already launched by the first wave.
    # Failures are handled by a later explicit rerun after summarization.
    if log.exists():
        return True
    return False
...
def gpu_free(gpu: int) -> bool:
    ...
    threshold = int(os.environ.get("FUSIONRAG_QWEN3_CROSS_RATE_SWEEP_FREE_MEM_MB", "10000"))
...
def cache_path(host: str, gpu: int, dataset: str) -> Path:
...
def run_task(host: str, gpu: int, task: dict) -> int:
    ...
    cmd = [
        ...
        "--cache_path", str(cache_path(host, gpu, task["dataset"])),
...
def worker(gpu: int, tasks: list[dict]) -> None:
    ...
    with log_path.open("a", encoding="utf-8", buffering=1) as log:
        ...
        for task in tasks:
            label = f"{task['dataset']} {task['method']} rate={task['rate']} {task['start']}-{task['end']}"
            if done(task):
