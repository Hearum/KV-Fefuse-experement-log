...
p.add_argument("--train-contexts", type=int, default=40)
p.add_argument("--test-contexts", type=int, default=10)
p.add_argument("--only-context-index", type=int)
a = p.parse_args()
...
rng = random.Random(17000 + 1000 * a.example + a.target)
rng.shuffle(pool)
train_pool, test_pool = sorted(pool[:5]), sorted(pool[5:])
assert set(train_pool).isdisjoint(test_pool)
train_ctxs = contexts(train_pool, a.train_contexts, seed=21000 + 1000 * a.example + a.target)
test_ctxs = contexts(test_pool, a.test_contexts, seed=31000 + 1000 * a.example + a.target)
ctxs = [("train", x) for x in train_ctxs] + [("test", x) for x in test_ctxs]
if a.only_context_index is not None:
    ctxs = [ctxs[a.only_context_index]]
...
manifest = []
for local_index, (split, prefix_ids) in enumerate(ctxs):
    context_index = a.only_context_index if a.only_context_index is not None else local_index
    ...
    for source, cache_dir in (
        ...
        payload = {
            ...
            "meta": {
                ...
                "context_index": context_index, "split": split, "prefix_ids": list(prefix_ids),
...
if a.only_context_index is None:
    (out / "manifest.json").write_text(json.dumps({
        "part": a.part, "example": a.example, "target": a.target,
        "contexts": len(ctxs), "train_pool": train_pool, "test_pool": test_pool,
        "train_contexts": [list(x) for x in train_ctxs],
        "test_contexts": [list(x) for x in test_ctxs],
        "runs": manifest,
    }, indent=2) + "\n")
