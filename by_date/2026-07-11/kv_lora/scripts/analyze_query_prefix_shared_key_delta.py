...
from MOTIVATION_EXPERIMENTS.kv_lora.scripts.analyze_delta_kv_sanity import _apply_rope_position_delta
...
def capture_key(x,doc_start):
 ...
 return torch.stack([v[:,:,doc_start:doc_start+doc_len,:].detach().cpu().to(torch.bfloat16) for v in cache.key_cache])
full=[]
for p in prefixes:full.append(capture_key(torch.cat([sy,p]+docs),len(sy)+len(p)))
...
def load_base(subdir):
 ...
 for i,doc in zip(ids,docs):
  key=torch.load(root/subdir/f"0_{i}_key.pt",weights_only=True,map_location="cpu")
  vals.append(_apply_rope_position_delta(m,key,shift,D).to(torch.bfloat16)); shift+=len(doc)
...
tag="qwen3_32b_ex0_query_prefix_fixedlen_key_shared";fields=list(rows[0]);
