...
mkdir -p "$EXP/logs" "$EXP/results" "$EXP/locks" "$CACHE"
LOG="$EXP/logs/trivia_gpu47_supervisor.log"
TASKS="$EXP/logs/tasks_trivia_gpu47.tsv"
...
run_one_task() {
  ...
  local lock_dir="$EXP/locks/${cfg}_${dataset}_${start}_${end}.lock"
  mkdir -p "$out_dir"
  ...
  if ! mkdir "$lock_dir" 2>/dev/null; then
    log "skip locked cfg=$cfg dataset=$dataset segment=$start-$end gpu=$gpu"
    return 0
  fi
...
run_workers() {
  make_tasks
  log "launching $WORKER_COUNT trivia workers on qjy003 gpu_list=$GPU_LIST cache=$CACHE"
  local rank=0
  for gpu in "${GPUS[@]}"; do
    nohup "$0" --worker "$rank" "$gpu" > "$EXP/logs/trivia_gpu${gpu}.outer.log" 2>&1 < /dev/null &
    echo $! > "$EXP/logs/trivia_gpu${gpu}.pid"
    log "launched worker rank=$rank gpu=$gpu pid=$(cat "$EXP/logs/trivia_gpu${gpu}.pid")"
    rank=$((rank + 1))
  done

  while true; do
    local running
    running=$(ps -ef | { grep "launch_ragdocs_ablation_trivia_gpu47_qjy003.sh --worker" || true; } | { grep -v grep || true; } | wc -l)
  ...
  log "trivia gpu47 supervisor done"
}
...
log "trivia gpu47 supervisor start host=$(hostname)"
run_workers
