...
OUT = Path(os.environ.get('SETUP_V2_REJUDGE_OUT_DIR', str(EXP / 'rejudge_glm_clean_20260714')))
RESULTS_ROOT = Path(os.environ.get('SETUP_V2_REJUDGE_RESULTS_ROOT', str(EXP / 'results')))
...
def load_complete_keys():
    if not COMPLETE_ONLY:
        return None
    summary_path = EXP / 'setup_v2_summary.csv'
    if not summary_path.exists():
        raise FileNotFoundError(f'complete-only rejudge requires {summary_path}')
    complete = set()
    with summary_path.open('r', encoding='utf-8', newline='') as handle:
        for row in csv.DictReader(handle):
            if str(row.get('complete', '')).lower() == 'true':
                complete.add((row.get('dataset', ''), row.get('method', ''), row.get('rate', '')))
    return complete

def iter_prediction_rows():
    rows = []
    complete_keys = load_complete_keys()
    for path_str in sorted(glob.glob(str(RESULTS_ROOT / '*/*/rate_*/*/csv/reprocess_method_*.csv'))):
        path = Path(path_str)
        method, dataset, rate_tag, segment = path.relative_to(RESULTS_ROOT).parts[:4]
