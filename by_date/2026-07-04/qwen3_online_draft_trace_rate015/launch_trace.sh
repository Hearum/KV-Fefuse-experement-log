...
OUT_ROOT="$REPO/MOTIVATION_EXPERIMENTS/qwen3_online_draft_trace_rate015"
TRACE_DIR="$OUT_ROOT/selected_indices"
...
run_segment(){
  ...
  mkdir -p "$out_dir" "$OUT_ROOT/logs" "$TRACE_DIR"
  echo "[$(date '+%F %T')] start gpu=$gpu segment=$start-$end" | tee -a "$OUT_ROOT/logs/seg_${start}_${end}.log"
  cd "$REPO"
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 FUSIONRAG_SAVE_SELECTED_DIR="$TRACE_DIR" "$PY" test_fusionrag_reflect_preprocess_exp.py \
    --model_type qwen3 \
    --model_path "$MODEL" \
...
cd "$REPO"; mkdir -p "$OUT_ROOT/logs" "$TRACE_DIR"
