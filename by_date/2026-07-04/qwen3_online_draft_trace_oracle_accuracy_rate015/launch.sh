...
OUT_ROOT="$REPO/MOTIVATION_EXPERIMENTS/qwen3_online_draft_trace_oracle_accuracy_rate015"
FIXED="$REPO/MOTIVATION_EXPERIMENTS/offline_fixed_token_selector/online_draft_trace_oracle_rate015/chunk_fixed_sets_npz"
...
run_segment(){
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 "$PY" test_fusionrag_reflect_preprocess_exp.py \
    ...
    --cache_path "$CACHE" --result_path "$out_dir" --start_sample "$start" --end_sample "$end" \
    --rate 0.15 --topk 10 --preprocess true --recall_method bge --reprocess_method FusionRAG \
    --revert_rope true --preprocess_scope global --bge_model_path "$BGE" --device cuda:0 --use_multi_gpu false \
    --openai_base_url "$API_URL" --openai_api_key "$API_KEY" --openai_model GLM-5.2 \
    --offline_fixed_set_dir "$FIXED" --offline_fixed_set_method online_draft_trace_frequency_per_chunk --offline_fixed_set_rate 0.15 \
    > "$out_dir/run.log" 2>&1
