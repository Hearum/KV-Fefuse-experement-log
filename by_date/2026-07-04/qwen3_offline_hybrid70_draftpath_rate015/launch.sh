...
OUT_ROOT="$REPO/MOTIVATION_EXPERIMENTS/qwen3_offline_hybrid70_draftpath_rate015"
...
run_segment(){
  local gpu="$1" start="$2" end="$3"; local out_dir="$OUT_ROOT/seg_${start}_${end}"
  ...
  CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 "$PY" test_fusionrag_reflect_preprocess_exp.py \
    --model_type qwen3 --model_path "$MODEL" --model_name Qwen3-32B --draft_model_path "$DRAFT" \
    --data_path ./data/result_reflect.json --dataset_name musique --cache_path "$CACHE" --result_path "$out_dir" \
    --start_sample "$start" --end_sample "$end" --rate 0.15 --topk 10 --preprocess true --recall_method bge \
    --reprocess_method DraftModel --revert_rope true --preprocess_scope global --bge_model_path "$BGE" --device cuda:0 --use_multi_gpu false \
    --openai_base_url "$API_URL" --openai_api_key "$API_KEY" --openai_model GLM-5.2 \
    --offline_fixed_set_dir "$FIXED" --offline_fixed_set_method hybrid_draft70_qk30_score_per_chunk --offline_fixed_set_rate 0.15 \
    > "$out_dir/run.log" 2>&1
